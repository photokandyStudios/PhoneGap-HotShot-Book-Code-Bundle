 ...
 *
 */
DOCS.PathRecDocumentCollection = function ( completion, failure )
{
    var self = this;
    ...
    self.dispatchFailure = function ( e )
    {
        ...
        if (self.failure)
        {
            ...
            PKUTIL.delay (500, function () { self.failure ( e ) });
        }
    }
    ...
    self.getDocumentsSuccess = function ( entries )
    {
        ...
        for (var i=0; i<entries.length; i++)
        {
            ...
            if (entries[i].isFile)
            {
                ...
                if (theFileType === APP.globalFileExtension)
                {
    ...
    self.createDocument = function ( theDocumentName, completion, failure )
    {
        ...
        // now create the file
        self.fileSystem.root.getFile ( theDocumentName.trim(), {create: true, exclusive: false},
    ...
    self.openDocumentAtIndex = function ( idx, completion, failure )
    {
        ...
        self.fileSystem.root.getFile ( self.documents[idx].name.trim(), {create: false, exclusive: false},
                                       function ( theFileEntry )
                                       {
                                           self.fileEntry = theFileEntry;
                                           self.state = "";
