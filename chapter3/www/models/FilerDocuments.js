...
DOCS.Filers = function ( completion, failure )
{
    ...
    self.getDocumentsSuccess = function ( entries )
    {
        ...
        for (var i=0; i<entries.length; i++)
        {
            ...
            if (entries[i].isFile)
            {
                ...
                var theFileType = theFileName.substr(theFileName.length-4,4);
                if (theFileType === ".fln")
                {
    ...
    self.renameDocumentAtIndexTo = function ( idx, newName, completion, failure )
    {
        ...
        self.documents [ idx ].moveTo ( self.fileSystem.root, newName, self.renameDocumentAtIndexToSuccess, self.dispatchFailure);
    }
    ...
    self.copyDocumentAtIndexTo = function ( idx, newName, completion, failure )
    {
        ...
        self.documents [ idx ].copyTo ( self.fileSystem.root, newName, self.copyDocumentAtIndexToSuccess, self.dispatchFailure);
    }
    ...
    self.createDocument = function ( theDocumentName, completion, failure )
    {
        ...
        self.fileSystem.root.getFile ( theDocumentName, {create: true, exclusive: false},
    ...
    self.openDocumentAtIndex = function ( idx, completion, failure )
    {
        ...
        self.fileSystem.root.getFile ( self.documents[idx].name, {create: false, exclusive: false},
                                       function ( theFileEntry )
                                       {
                                           self.fileEntry = theFileEntry;
                                           self.state = "";
