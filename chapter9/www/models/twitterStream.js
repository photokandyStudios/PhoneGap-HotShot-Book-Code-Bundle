    ...
    self.setScreenName = function(theScreenName, completion) {
        ...
        PKUTIL.loadJSON(getUserURL, function(success, data) {
            if (success) {
                ...
                self._userData = {
                    "error": "Twitter error; rate limited?"
                };
            }
            if (completion) {
                completion();
...
TWITTER.TwitterStream = function(theScreenNameOrSearchPhrase, completion, theMaxCount) {
    ...
    self.loadStream = function(completion) {
        ...
        if (self._searchPhrase.substr(0, 1) == "@") {
            theStreamURL = TWITTER._baseURL + "statuses/user_timeline.json?include_entities=true&include_rts=true&count=" + self._theMaxCount + "&screen_name=" + encodeURIComponent(self._searchPhrase);
            forScreenName = true;
        } else {
            theStreamURL = TWITTER._searchBase + "search.json?q=" + encodeURIComponent(self._searchPhrase) + "&include_entities=true" + "&include_rts=true&rpp=" + self._theMaxCount;
